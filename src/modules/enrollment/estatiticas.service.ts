...
export class EstudantesService {
  ...
  async findEstudantes(estudanteDto: EstudanteDTO) {
    ...
    const baseQuery = this.dataSource
      ...
      .innerJoin('FK2_TB_ANO_LECTIVO', 'TAL', 'TAL.CODIGO = :anoLectivo', {
        anoLectivo,
      })

      // Garante que só buscamos alunos com registos no ano selecionado
      .where(
        'EXISTS (SELECT 1 FROM FK2_TB_GRADE_CURRICULAR_ALUNO X WHERE X.CODIGO_MATRICULA = TM.CODIGO AND X.CODIGO_ANO_LECTIVO = :anoLectivo)',
        { anoLectivo },
      );
    ...
    if (genero && genero.toLowerCase() !== 'todos')
      baseQuery.andWhere('UPPER(TP.SEXO) = :genero', {
        genero: genero.toUpperCase(),
      });
    if (search)
      baseQuery.andWhere('UPPER(TP.Nome_Completo) LIKE UPPER(:search)', {
        search: `%${search}%`,
      });
    ...
    const promessas = rawResults.map(async (raw) => {
      ...
      if (anoCurricular && Number(item.classe_id) !== Number(anoCurricular))
        return null;
      ...
      if (
        estadoMatricula &&
        estadoMatricula !== 0 &&
        situacao?.codigo !== estadoMatricula
      )
        return null;
    ...
    const resultadosFiltrados = (await Promise.all(promessas)).filter(
      (res) => res !== null,
    );
  ...
  private async estadoMatricula(matriculaEstudante: number) {
    ...
    const estadoMatriculaStr =
      await this.obterStatusMatricula(matriculaEstudante);
    ...
    if (estadoMatriculaStr?.toUpperCase() === 'DIPLOMADO')
      return await this.matriculaSituacaoBySigla('E');

    if (estadoMatriculaStr?.toUpperCase() === 'INACTIVO') {
      const mesTemp = await this.buscarUltimaParcelaPagaMesTemp(
        matriculaEstudante,
        anoLectivo.codigo,
      );
      if (mesTemp) {
        ...
        if (await this.findBolsa100p(matriculaEstudante, anoAnterior.codigo))
          return await this.matriculaSituacaoBySigla('AI');
      }
    ...
    if (
      tipoCandidatura === 1 &&
      (await this.temConfirmacao(matriculaEstudante, anoLectivo.codigo))
    ) {
      const mesTemp = await this.buscarUltimaParcelaPagaMesTemp(
        matriculaEstudante,
        anoLectivo.codigo,
      );
      if (mesTemp && mesTemp.prestacao >= dadosMesCorrente?.nPrestacao)
  ...
  private async matriculaSituacaoBySigla(sigla: string) {
    const situacao = await this.dataSource
      .createQueryBuilder()
      .select('*')
  ...
  private async buscarMesCorrente(data: Date) {
    ...
    const res = await this.dataSource
      ...
      .getRawOne();
    return res
      ? { dataLimiteDoMesCorrente: res.data_limite, nPrestacao: res.prestacao }
      : null;
  }

  private async buscarUltimaParcelaPagaMesTemp(
    numMatricula: number,
    anoLectivo: number,
  ) {
  ...
  async seTemIsencao(
    ...
    const res = await this.dataSource
      ...
      .where(
        'TI.CODIGO_MATRICULA = :numMatricula AND TI.CODIGO_ANOLECTIVO = :anoLectivo',
        { numMatricula, anoLectivo },
      )
      .andWhere("TI.ESTADO_ISENSAO = 'Activo' AND MT.PRESTACAO <= :prestacao", {
        prestacao,
      })
      .getRawOne();
  ...
  async findBolsa100p(matricula: number, anoLectivo: number): Promise<boolean> {
    const res = await this.dataSource
      ...
      .where('TB.CODIGO_MATRICULA = :matricula AND TB.STATUS_ = 0', {
        matricula,
      })
      .andWhere(
        'TB.DESCONTO IN (0, 100) AND TB.CODIGO_ANOLECTIVO = :anoLectivo',
        { anoLectivo },
      )
      .getRawOne();
  ...
  async temConfirmacao(
    ...
    const res = await this.dataSource
      ...
      .where(
        'TGCA.CODIGO_STATUS_GRADE_CURRICULAR IN (2, 3) AND TGCA.CODIGO_ANO_LECTIVO = :anoLectivo',
        { anoLectivo },
      )
      .andWhere(
        'TGCA.CODIGO_MATRICULA = :numMatricula AND TGCA.EQUIVALENCIA = 0',
        { numMatricula },
      )
      .getRawOne();
    return Number(res?.total) > 0;
