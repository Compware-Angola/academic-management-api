...
describe('StudentNoteService - processarNotasHorario', () => {
  ...
  const baseGradeAluno = (overrides: Record<string, any> = {}) => ({
    ...
    EQUIVALENCIA: 0,
    NOTA: null,
    PESO_PRIMEIRA_FREQ: 50,
    PESO_SEGUNDA_FREQ: 50,
    NOTA_MIN_PRIMEIRA_FREQ: 0,
    NOTA_MIN_SEGUNDA_FREQ: 0,
    ...overrides,
  });
  ...
  beforeEach(() => {
    ...
    service = new StudentNoteService(
  ...
  it('1) ano lectivo anterior ao activo com NOTA >= 10 aprova sem calcular fórmula', async () => {
    ...
    const gradeAluno = baseGradeAluno({
      CODIGO_ANO_LECTIVO: ANO_ATUAL - 1,
      NOTA: 12,
    });

    const pauta = await (service as any).processarNotasHorario(
      100,
      false,
      gradeAluno,
    );
  ...
  describe('fluxo de acordo com o plano curricular', () => {
    beforeEach(() => {
      jest.spyOn(service as any, 'temPrazo').mockResolvedValue(false);
      jest
        .spyOn(service as any, 'findOnePlanoByCursoAndAnoLectivo')
    ...
    it('4) TEM_ORAL=true, oral ausente entra como 0 e, se insuficiente, cai no Recurso (também sem notas = 0)', async () => {
      ...
      // (9+9+0)/3=6 na frequência -> insuficiente -> RECURSO; sem notas de
      // recurso lançadas, (0+0)/2=0 -> REPROVADO
    ...
    it('4b) TEM_ORAL=true, nota oral baixa mas média combinada (1ªF+2ªF+Oral)/3 aprova', async () => {
      jest
        .spyOn(service as any, 'buscarAvaliacoes')
        .mockResolvedValue([avaliacao(2, 12), avaliacao(3, 12), avaliacao(9, 6)]);
      jest
        .spyOn(service as any, 'findByPlanoAndUnidadeCurricular')
        .mockResolvedValue({ TEM_ORAL: true, TEM_PRATICA: false });
    ...
    it('5) TEM_PRATICA=true, prática ausente entra como 0 e reprova sem direito a recurso', async () => {
      ...
      // mediaFreq=10 >=9.5 -> AGUARDA_PRATICA; prática ausente -> round((10+10+0)/3)=7
      // insuficiente -> RECURSO (nota seca, prática não volta a contar); sem
      // nota de recurso lançada -> REPROVADO
    ...
    it('5b) TEM_PRATICA=true, nota prática baixa mas média combinada (1ªF+2ªF+Prática)/3 aprova', async () => {
      jest
        .spyOn(service as any, 'buscarAvaliacoes')
        .mockResolvedValue([avaliacao(2, 12), avaliacao(3, 12), avaliacao(4, 6)]);
      jest
        .spyOn(service as any, 'findByPlanoAndUnidadeCurricular')
        .mockResolvedValue({ TEM_ORAL: false, TEM_PRATICA: true });
      ...
      // (12+12+6)/3 = 10, mesmo com a nota da prática (6) abaixo de 10
      expect(pauta.media).toBe('10');
      expect(pauta.resultado).toBe(EstadoAvaliacaoEnum.APROVADO);
    });
    ...
    it('7) TEM_ORAL=true e TEM_PRATICA=true (cadastro incorrecto) prioriza oral e regista aviso', async () => {
      ...
      expect(
        pauta.obs.some((o: string) => o.includes('Prova Oral')),
      ).toBe(true);
      expect(
        pauta.obs.some((o: string) => o.includes('Prática')),
      ).toBe(false);
    });
  });
  ...
  // REGRA 4 — cálculo de recurso oral combinado --------------------------------------
  ...
  describe('Recurso com Oral/Prática', () => {
    ...
    it('11) Recurso com Oral combina sempre, mesmo com nota de recurso ausente', async () => {
      jest
        .spyOn(service as any, 'buscarAvaliacoes')
        .mockResolvedValue([avaliacao(2, 6), avaliacao(3, 6), avaliacao(23, 20)]);
      jest
        .spyOn(service as any, 'findByPlanoAndUnidadeCurricular')
        .mockResolvedValue({ TEM_ORAL: true, TEM_PRATICA: false });
    ...
    it('12b) Prática insuficiente cai em Recurso (nota seca) e pode aprovar', async () => {
      jest
        .spyOn(service as any, 'buscarAvaliacoes')
        .mockResolvedValue([avaliacao(2, 6), avaliacao(3, 6), avaliacao(7, 14)]);
      jest
        .spyOn(service as any, 'findByPlanoAndUnidadeCurricular')
        .mockResolvedValue({ TEM_ORAL: false, TEM_PRATICA: true });
