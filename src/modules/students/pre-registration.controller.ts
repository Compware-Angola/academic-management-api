...
export class PreRegistrationController {
  ...
  @ApiResponse({
    status: 200,
    description: 'Lista de pré-inscrições retornada',
  })
  ...
  @ApiResponse({
    status: 200,
    description: 'Pré-inscrição atualizada com sucesso',
  })
  @ApiResponse({ status: 404, description: 'Pré-inscrição não encontrada' })
  @ApiResponse({
    status: 409,
    description: 'BI ou Email já em uso por outro registo',
  })
  update(
  ...
  @ApiResponse({
    status: 204,
    description: 'Pré-inscrição removida com sucesso',
  })
  @ApiResponse({ status: 404, description: 'Pré-inscrição não encontrada' })
  ...
  @ApiParam({
    name: 'userId',
    type: Number,
    description: 'ID do utilizador (FK2_USERS.ID)',
  })
  ...
  @ApiOperation({
    summary:
      'Informações gerais do candidato (pre-inscrição + admissao + matricula + prova)',
    description:
