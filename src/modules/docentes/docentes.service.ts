...
async teacherWeeklySchedule(dto: FindTeacherWeeklyScheduleDto) {
  const docente = Number(dto.docente);
  const anoLectivo = Number(dto.anoLectivo);
  const semestre = Number(dto.semestre);
  const periodo = dto.periodo ? Number(dto.periodo) : 0;

  const sql = `
    SELECT
      a.PK_AULA AS codigo,
      ds.PK_DIA_DA_SEMANA AS dia_semana,
      ds.DESIGNACAO AS dia_semana_designacao,
      ds.ORDEM AS dia_semana_ordem,
      a.ORDEM AS ordem,
      TO_CHAR(a.HORA_INICIO, 'HH24:MI') AS hora_inicio,
      TO_CHAR(a.HORA_TERMINO, 'HH24:MI') AS hora_fim,
      JSON_VALUE(a.REF_AULA, '$.desc') AS disciplina,
      JSON_VALUE(a.REF_DOCENTE, '$.nome') AS docente,
      JSON_VALUE(a.REF_SALA, '$.desc') AS sala,
    ...
    INNER JOIN FK2_MGH_TB_DIA_DA_SEMANA ds
      ON a.FK_DIA_DA_SEMANA = ds.PK_DIA_DA_SEMANA
    LEFT JOIN FK2_MGH_TB_MODALIDADE m
      ON a.FK_MODALIDADE = m.PK_MODALIDADE
    LEFT JOIN FK2_MGH_TB_TIPO_AULA ta
      ON a.FK_TIPO_AULA = ta.PK_TIPO_AULA
    WHERE h.ACTIVE_STATE = 1
      AND h.FK_ESTADO_HORARIO_WF NOT IN (4)
      AND a.ACTIVE_STATE = 1
      AND JSON_VALUE(h.REF_ANO_LECTIVO, '$.pk' RETURNING NUMBER) = :anoLectivo
      AND JSON_VALUE(h.REF_PERIODICIDADE, '$.pk' RETURNING NUMBER) = :semestre
      AND (:periodo = 0 OR JSON_VALUE(h.REF_PERIODICIDADE, '$.pkPeriodo' RETURNING NUMBER) = :periodo)
      AND JSON_VALUE(a.REF_DOCENTE, '$.pkDocente' RETURNING NUMBER) = :docente
    ORDER BY ds.ORDEM, a.ORDEM
  `;
  ...
  try {
    ...
    return {
      ...
      resumo: {
        ...
        aulasPresenciais: data.filter((item: any) =>
          String(item.modalidade || '').toLowerCase().includes('presencial')
        ).length,
        aulasVirtuais: data.filter((item: any) =>
          String(item.modalidade || '').toLowerCase().includes('virtual')
        ).length,
      },
    };
  } catch (error) {
    console.error('Erro ao buscar horário semanal do docente:', error);
    throw new Error(`Falha ao consultar horário semanal: ${error.message}`);
  }
}
}