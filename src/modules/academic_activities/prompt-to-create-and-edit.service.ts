...
export class promptToCreateAndEditService {
  ...
  private async initAnoAtual() {
    ...
    this.semestre = (await this.anoLectivoUtil.getSemestreAtual()).semestre ?? 1;
  }
  ...
  async promptToCreateAndEditSchedule(
    ...
    const promptToCreateAndEdit = await this.promptToCreateAndEdit(TypePromptEnum.CRIACAO_HORARIO, ano_lectivo, semestre || this.semestre);
  ...
  private async promptToCreateAndEdit(
    ...
    SELECT 
        ...
        p.OBSERVACAO, 
        p.FK_TIPO_PRAZO, 
        tp.DESIGNACAO AS TIPO_PRAZO_NOME,
        ...
        CASE
          WHEN SYSDATE BETWEEN p.DATA_INICIO AND p.DATA_FIM
          THEN 1
          ELSE 0
        END AS IS_IN_PRAZO

    FROM FK2_MCAL_TB_PRAZO p
    LEFT JOIN FK2_MCAL_TB_TIPO_PRAZO tp 
      ON tp.PK_TIPO_PRAZO = p.FK_TIPO_PRAZO

    LEFT JOIN FK2_MCAL_TB_TIPO_AVALIACAO av 
      ON av.PK_TIPO_AVALIACAO = p.FK_TIPO_AVALIACAO
  ...
  `;
    ...
    const params: any = {
      sigla_prazo,
      ano_lectivo: ano_lectivo || this.anoAtualPrincipal,
    };
    ...
    return {
      ...result[0],
      IS_IN_PRAZO: result[0].IS_IN_PRAZO === 1,
    };
  }
