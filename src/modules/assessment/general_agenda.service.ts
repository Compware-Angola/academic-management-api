    ...
    try {
        if (horario) {
            ...
            if (grade) {
                ...
                console.log(listaPauta,"22222222222222222222222222");
...
private async carregarPautaHorario(grade: any, scheduleId: number, anoCorrente: number): Promise<any[]> {
    const pautaGeral: any[] = [];  // ← array para acumular as pautas dos alunos
    ...
    return pautaGeral;  // ← agora retorna a lista completa
}
...
private async retornarGradeAvaliadaByGrade(
    ...
    private async findByPlanoAndUnidadeCurricular(plano: number, codigoUnidadeCurricular: number): Promise<any> {
        ...
        console.log(plano, codigoUnidadeCurricular, "SSSSSSSSSSSSSSS");
 ...
 private async processarNotasHorario(codigoMatricula: number, gradeAluno: any): Promise<any> {
    ...
    private async temNota(avaliacao: any | null): Promise<boolean> {
        return avaliacao !== null && avaliacao?.NOTA !== undefined && avaliacao?.NOTA !== null;
    }
...
`, [gradeCurricular]);
    ...
    SELECT DISTINCT avaliacoes
  FROM FK2_TB_GRADE_CURRICULAR_ALUNO grade
   INNER JOIN FK2_TB_GRADE_CURRICULAR_ALUNO_AVALIACOES
    avaliacoes WHERE avaliacoes.TIPO_AVALIACAO = :tipoavaliacao 
...
`, [tipoavaliacao, turma, gradeCurricular]);
        ...
        return result ? result === true || result === 1 : false;
