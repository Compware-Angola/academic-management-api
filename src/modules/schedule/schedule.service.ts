    ...
    FROM (
      ...
      FROM (
        SELECT
          ...
          TO_CHAR(h."UPDATED_AT", 'DD/MM/YYYY HH24:MI')                         AS "DATAULTIMAATUALIZACAO",
          TO_CHAR(h."CREATED_AT", 'DD/MM/YYYY HH24:MI')                         AS "DATACRIACAO",
          -- ✅ FIX 2: COUNT com os mesmos filtros da query de alunos
        ...
        -- ✅ FIX 3: Subquery com filtros consistentes (status <> 5)
        LEFT JOIN (
          SELECT
            JSON_VALUE(GCA."REF_HORARIO", '$.pk' RETURNING NUMBER) AS "HORARIO_ID",
            COUNT(*) AS "TOTAL_ALUNOS"
          FROM "FK2_TB_GRADE_CURRICULAR_ALUNO" GCA
          LEFT JOIN "FK2_TB_CONFIRMACOES" CONF
            ON CONF."CODIGO" = GCA."CODIGO_CONFIRMACAO"
          WHERE GCA."CODIGO_STATUS_GRADE_CURRICULAR" <> 5
          GROUP BY
            JSON_VALUE(GCA."REF_HORARIO", '$.pk' RETURNING NUMBER)
        ) alu
          -- ✅ FIX 4: JOIN só pelo HORARIO_ID (sem CODIGO_GRADE_CURRICULAR que causava duplicatas)
          ON alu."HORARIO_ID" = h."PK_HORARIO"
