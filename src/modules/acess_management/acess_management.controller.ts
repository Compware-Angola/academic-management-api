...
import { 
  ApiNotFoundResponse, ApiOperation, ApiResponse,
  ApiBearerAuth,
  ApiParam,
  ApiQuery,
...
export class AcessManagementController {
  ...
  @ApiOperation({
    ...
    description: 'Retorna apenas os acessos ativos atribuídos ao utilizador via grupo (incluindo grupo unitário).',
  })
  @ApiParam({ name: 'id', description: 'ID do utilizador (PK_UTILIZADOR)', example: 123 })
  @ApiResponse({ status: 200, type: [AcessoResponseDto] })
  @ApiNotFoundResponse({ description: 'Utilizador não encontrado ou sem acessos' })
  @ApiUnauthorizedResponse({ description: 'Não autenticado' })
  ...
  @ApiOperation({
    ...
    description: 'Útil para visualizar permissões de grupos comuns ou unitários.',
  })
  ...
  @ApiOperation({
    ...
    description: 'Se o acesso estiver removido, reativa. Caso contrário, adiciona ao grupo unitário.',
  })
  ...
  @ApiResponse({ status: 201, description: 'Acesso adicionado/reativado com sucesso' })
  @ApiBadRequestResponse({ description: 'Dados inválidos ou acesso já existente' })
  @ApiNotFoundResponse({ description: 'Utilizador, grupo ou acesso não encontrado' })
  @ApiUnauthorizedResponse({ description: 'Não autenticado' })
  ...
  @ApiOperation({
    ...
    description: 'Marca o acesso como removido para o grupo unitário do utilizador.',
  })
  @ApiParam({ name: 'utilizadorId', description: 'ID do utilizador alvo' })
  @ApiParam({ name: 'acessoId', description: 'ID do acesso a ser removido' })
  @ApiResponse({ status: 200, description: 'Acesso removido/revogado com sucesso' })
  @ApiNotFoundResponse({ description: 'Utilizador, grupo ou acesso não encontrado' })
  @ApiUnauthorizedResponse({ description: 'Não autenticado' })
  async removerAcesso(
    @Param('utilizadorId', ParseIntPipe) utilizadorId: number,
